...
```
#R^2 = 0.0233. Since the R^2 value is so low this tells us that these two factors are not strongly linearly related. Also there are really significant Coefficients within the model baded on the summary. 
```
...
ggplot(Fish_Data, aes(x = SL, y = LL)) + 
    ...
    labs(x = "Standard Length (mm)", 
         y = "Lateral Line Length", 
         title = "Linear regression to predict Standard Length") + 
    annotate("text",                
             x = 35, y = 45,         
             label = "R^2 == 0.0233 ", 
             parse=T) +
...
```{r}
#just do another model SL and CPD? -- No I don't think its possible based on the data given. Here is an example based on the two other numerical factors that are in my given dtataset. Again the low R^2 value given from this tells me that this data is not linearly related  (orginal vs new one facet them togetht)
...
summary(anova_model)
# add means or eorror  bas but this deos tell as tory that the feamles are males are not preportal so it violtesa the rule 
...
```
#A linear model provides more insight into the relationship and fit between the variables by showing you the R^2 value. 
